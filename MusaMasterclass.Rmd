...
The default is to look for flat areas 1/400th the size of the matrix. There's nothing special about this number, but it's a default that tended to work fairly well for several datasets I tested it on, but it's a parameter you can adjust to 

```{r}

hobart_mat %>%
  sphere_shade() %>%
  add_water(detect_water(hobart_mat), color="blue") %>%
  plot_map()

```

Rayshader's name comes from the fact that it uses raytracing to simulate light across the DEM. Let's add a layer of shadows to this map, using the `add_shadow()` and `ray_shade()` functions. 

![Figure X: Demonstration of how ray_shade() raytraces images](images/ray.gif)

We use the pipe to add a shadow layer to our color layer. 

```{r}

hobart_mat %>%
  ray_shade(zscale=33, sunaltitude = 3, lambert = FALSE) %>%
  plot_map()

```

Rayshader does this by drawing rays originating from each point towards a light source, specified using the `sunangle` and `anglebreaks` argument. 
...
hobart_mat %>%
  ...
  add_shadow(ray_shade(hobart_mat,zscale=33, sunaltitude = 3, lambert = FALSE), 
             max_darken = 0) %>%
  plot_map()
...
hobart_mat %>%
  ...
  add_shadow(ray_shade(hobart_mat,zscale=33, sunaltitude = 3, lambert = FALSE), 
             max_darken = 0.3) %>%
  add_shadow(lamb_shade(hobart_mat,zscale=33,sunaltitude = 3), max_darken = 0.3) %>%
  plot_map()
...
We can adjust the highlight direction using the `sunangle` argument in both the `sphere_shade()` function and the `ray_shade()` function.
...
hobart_mat %>%
  sphere_shade() %>%
  add_water(detect_water(hobart_mat), color="lightblue") %>%
  add_shadow(ray_shade(hobart_mat), max_darken = 0.5) %>%
  plot_map()
...
hobart_mat %>%
  ...
  add_water(detect_water(hobart_mat), color="lightblue") %>%
  add_shadow(ray_shade(sunangle = 45, hobart_mat), max_darken = 0.5) %>%
  plot_map()
...
hobart_mat %>%
  ...
  add_water(detect_water(hobart_mat), color="lightblue") %>%
  add_shadow(ray_shade(sunangle = 135, hobart_mat), max_darken = 0.5) %>%
  plot_map()
...
hobart_mat %>%
  ...
  add_water(detect_water(hobart_mat), color="lightblue") %>%
  add_shadow(ray_shade(sunangle = 225, hobart_mat), max_darken = 0.5) %>%
  plot_map()

```

We can also add the effect of ambient occlusion, which in cartography circles is sometimes called the "sky view factor." When light travels through the atmosphere, it scatters. The entire sky acts as a light source, so when less of the sky is visible (e.g. in a valley) it's darker than when the entire sky is visible (e.g. on a mountain ridge). 
...
hobart_mat %>%
  sphere_shade() %>%
  add_water(detect_water(hobart_mat), color="lightblue") %>%
  add_shadow(ray_shade(hobart_mat), max_darken = 0.3) %>%
  plot_map()
...
hobart_mat %>%
  ...
  add_water(detect_water(hobart_mat), color="lightblue") %>%
  add_shadow(ray_shade(hobart_mat), max_darken = 0.3) %>%
  add_shadow(ambient_shade(hobart_mat), max_darken = 0.3) %>%
  plot_map()

```

Now, let's take our plots 3D. In rayshader, we do that simply by swapping out `plot_map()` with `plot_3d()`, and adding the heightmap to the function call. We don't want to re-compute the shadows every time we replot the landscape, so lets save them to a variable.


```{r}

rayshadows = ray_shade(hobart_mat, sunangle = 315,zscale=2)
ambientshadows = ambient_shade(hobart_mat,zscale=2)

hobart_mat %>%
  sphere_shade() %>%
  add_water(detect_water(hobart_mat), color="lightblue") %>%
  add_shadow(rayshadows, max_darken = 0.3) %>%
  add_shadow(ambientshadows, max_darken = 0.3) %>%
...
#   sphere_shade(sunangle = 225) %>%
#   add_water(detect_water(hobart_mat), color="lightblue") %>%
#   add_shadow(ray_shade(sunangle = 225, hobart_mat, zscale = real_zscale / 3, anglebreaks = seq(5,10,by=0.1)), max_darken = 0.5) %>%
#   plot_3d(hobart_mat, zscale = real_zscale / 3)
